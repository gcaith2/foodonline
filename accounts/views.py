...
# Restrict the customer to access venor page
...
def registerUser(request):
    if request.user.is_authenticated:
        ...
        return redirect('myAccount') #need to b corrected
...
def activate(request,uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active=True
        user.save()
        messages.success(request, 'Your account has been activated succesfully! Please login')
        return redirect('myAccount')
    else:
